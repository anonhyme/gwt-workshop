...
public class EditContactPresenterTest extends ClientTestCase {
  ...
  public void testContactsPresenter() throws Exception {
    ...
    editPresenter.setId(contactsPresenter.getContactDetails().get(0).getId());
    editPresenter.doSave();
    assertEquals("aName aName", contactsPresenter.getContactDetails().get(0).getDisplayName());
    
    // Add a new contact
    assertEquals(22, contactsPresenter.getContactDetails().size());
    eventBus.fireEvent(new AddContactEvent());
    editPresenter.doSave();
    assertEquals(23, contactsPresenter.getContactDetails().size());
